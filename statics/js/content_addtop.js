...
authkey 参数密钥，验证args
**/
function zipupload(uploadid, name, textareaid, funcName, args, module, catid, authkey) {
    var args = args ? '&args=' + args : '';
    var setting = '&module=' + module + '&catid=' + catid ;
    Wind.use("artDialog","iframeTools",function(){
        art.dialog.open(GV.DIMAUB+'index.php?a=zipupload&m=asset&g=asset' + args + setting, {
        title: name,
        id: uploadid,
        width: '650px',
        height: '420px',
...
//示例上传，SWF回调函数
function change_zip(uploadid, returnid) {
    var d = uploadid.iframe.contentWindow;
    var in_src = d.$("#att-status").html().substring(1);
    var in_filename = d.$("#att-name").html().substring(1);
    var in_filepath = d.$("#att-content").html().substring(1);
    var in_select = d.$("#att-select").html();
    var url = in_filepath.split('|');
    var src = in_src.split('|');
    var name = in_filename.split('|');
    //var filenames = in_filename.split('|');
    $('#source').val(src);//下载地址
    $('#description').val(url+'release/index.html');//演示地址
    $('#thumb').val(url+'logo.png');//图片
    $("#thumb_preview").attr("src", $("#thumb_preview").attr('data-site-root')+url+'logo.png');//图片
    $('#post_select').val(in_select);//
}
